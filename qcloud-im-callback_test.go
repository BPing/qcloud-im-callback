...
func TestFacade(t *testing.T) {
	...
	fmt.Println(ConsumerNumGoroutine())

}

func TestRegisterDefaultNewCallbackHandlerWithCache(t *testing.T) {
	cacheR:=&TestRedis{list.New()}
	defaultHandle := func(event *CallbackEvent) interface{} {
		return &BaseResponse{ActionStatus: OkStatus, ErrorCode: 0}
	}

	RegisterDefaultNewCallbackHandlerWithCache(0, 30, CallbackHandle(defaultHandle),cacheR)

	CallbackBeforeSendMsgHandle := func(event *CallbackEvent) interface{} {
		var sendMsgBody SendMsgBody
		event.ToJSON(&sendMsgBody)
		fmt.Println("RegisterDefaultNewCallbackHandlerWithCache: CallbackBeforeSendMsgHandle", sendMsgBody.MsgBody)
		return &BaseResponse{ActionStatus: OkStatus, ErrorCode: 0}
	}
	...
	if resp.(*BaseResponse).ActionStatus != OkStatus {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade")
	}

	resp = Handle(up.CallbackCommand, up, []byte(str))

	if resp.(*BaseResponse).ActionStatus != OkStatus {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade Handle")
	}

	resp = Handle(CallbackAfterSendMsgCommand, up, []byte(str))

	if resp.(*BaseResponse).ActionStatus != OkStatus {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade default Handle")
	}
	...
	if resp.(*BaseResponse).ActionStatus != FAILStatus {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade RegisterDefaultHandle")
	}
	...
	if resp.(*BaseResponse).ActionStatus != FAILStatus {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade RegisterBeforeHook 1")
	}
	...
	if resp.(*BaseResponse).ErrorCode != -1 {
		t.Fatal("RegisterDefaultNewCallbackHandlerWithCache: test facade RegisterBeforeHook 2", resp.(*BaseResponse).ErrorCode)
	}
